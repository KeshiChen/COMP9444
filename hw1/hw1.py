...
def my_relu(in_value):
    ...
    # out_value = tf.placeholder(dtype=tf.float32)
    # zero = tf.zeros(dtype=tf.float32, shape=[1])
    # tf.cond(in_value[0] > 0, tf.assign(out_value, in_value), tf.assign(out_value, zero))
    return out_value
...
def my_perceptron(x):
    ...
    w = tf.Variable(tf.ones(shape=[x], dtype=tf.float32))
    res = tf.reduce_sum(tf.multiply(i, w))
...
def onelayer(X, Y, layersize=10):
    ...
    """
    w = tf.Variable(tf.random_normal([int(X.shape[1]), layersize], stddev=0.1), dtype=tf.float32)
    b = tf.Variable(tf.zeros([layersize]), dtype=tf.float32)
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    :return: The following variables should be returned in the following order.
        ...
        batch_loss: The average cross-entropy loss of the batch
    """
    w1 = tf.Variable(tf.random_normal([int(X.shape[1]), hiddensize], stddev=0.1), dtype=tf.float32)
    b1 = tf.Variable(tf.zeros([hiddensize]), dtype=tf.float32)
    pred1 = my_relu(tf.matmul(X, w1) + b1)
    w2 = tf.Variable(tf.random_normal([hiddensize, outputsize], stddev=0.1), dtype=tf.float32)
    b2 = tf.Variable(tf.zeros([outputsize]), dtype=tf.float32)
    logits = tf.matmul(pred1, w2) + b2
    preds = tf.nn.softmax(logits)
    batch_xentropy = tf.nn.softmax_cross_entropy_with_logits(labels= Y, logits=logits)
    batch_loss = tf.reduce_mean(batch_xentropy)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    """
    conv1 = tf.layers.conv2d(inputs= X, kernel_size=filter_shape, filters=convlayer_sizes[0], padding=padding, activation=my_relu, kernel_initializer=tf.random_normal_initializer(stddev=0.01))
    conv2 = tf.layers.conv2d(inputs= conv1, kernel_size=filter_shape, filters=convlayer_sizes[1], padding=padding, activation=my_relu, kernel_initializer=tf.random_normal_initializer(stddev=0.01))
    conv2_reshape = tf.reshape(conv2, [-1, int(conv2.shape[1]) * int(conv2.shape[2]) * int(conv2.shape[3])]) # -1 means flatten
    w, b, logits, preds, batch_xentropy, batch_loss = onelayer(conv2_reshape, Y, layersize=outputsize)
