...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    b_in = tf.get_variable("b_in", shape=[1, hidden_nodes], initializer = tf.constant_initializer(0.1))
    # pred_in = tf.tanh(tf.matmul(state_in, w_in) + b_in)
    pred_in = tf.nn.relu(tf.matmul(state_in, w_in) + b_in)

    # hidden Layer 2
    w_hid = tf.get_variable("w_hid", shape=[hidden_nodes, hidden_nodes])
    b_hid = tf.get_variable("b_hid", shape=[1, hidden_nodes], initializer = tf.constant_initializer(0.1))
    # pred_in = tf.tanh(tf.matmul(pred_in, w_hid) + b_hid)
    pred_in = tf.matmul(pred_in, w_hid) + b_hid

    #pred_in = tf.layers.dropout(pred_in, 0.) 
    ...
    # select action with q_evaluate(q_target)
    q_selected_action = \
        tf.reduce_sum(tf.multiply(q_values, action_in), reduction_indices=1)
    
    # reg = 0.01 * (tf.reduce_sum(tf.square(w_in)) +tf.reduce_sum(tf.square(w_hid)) + tf.reduce_sum(tf.square(w_out)))
    #reg = 0.0001 * (tf.reduce_sum(tf.square(w_in)) + tf.reduce_sum(tf.square(w_out)))
    ...
    #optimise_step = tf.train.RMSPropOptimizer(0.01).minimize(loss)
...
def do_train_step(replay_buffer, state_in, action_in, target_in,
    ...
    target_batch, state_batch, action_batch = \
        get_train_batch(q_values, action_in, state_in, minibatch)
...
def get_train_batch(q_values, action_in, state_in, minibatch):
    """
