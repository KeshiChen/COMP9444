...
NUM_EPISODES = 2500  # Episode limitation
SAVE_FREQUENCY = 200  # How many episodes to run before saving model (unused)
EP_MAX_STEPS = 100  # Step limitation in an episode
...
def get_network(state_dim, action_dim, hidden_nodes=HIDDEN_NODES):
    ...
    with tf.variable_scope('q_val_net'):
        ...
        b_hid = tf.get_variable("b_hid", shape=[1, hidden_nodes], initializer = tf.constant_initializer(0.0))
...
def do_train_step(replay_buffer, state_in, action_in, target_in,
    ...
    target_batch, state_batch, action_batch = \
        get_train_batch(q_values, state_in, action_in, minibatch)
...
def get_train_batch(q_values, state_in, action_in, minibatch):
    """
    ...
    Q_value_batch = q_values.eval(feed_dict={
        state_in: next_state_batch, action_in: action_batch
    })
